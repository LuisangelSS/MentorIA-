...
// Cargar variables de entorno (para entornos que usen .env local)
...
// Chat endpoint and other functionality (kept as in original server)
app.post("/chat", validateToken, async (req, res) => {
    ...
    try {
        ...
        const contents = [
                {
                    ...
                    parts: [
                        {
                            text: `...` // trimmed for brevity in app file
                        },
                    ],
            }
        ];
        ...
        if (stream) {
            ...
            try {
                const stream = await ai.models.generateContentStream({ model: "gemini-2.5-flash", contents: contents });
                for await (const chunk of stream) {
                    const chunkText = chunk.candidates?.[0]?.content?.parts?.[0]?.text;
                    if (chunkText) {
                        fullResponse += chunkText;
                        if (isFirstChunk) {
                            res.write(`data: ${JSON.stringify({ type: 'start', chatSessionId: chatSession?.id, sessionName: chatSession?.session_name })}\n\n`);
                            isFirstChunk = false;
                        }
...
// Utilities (trimmed versions kept)
...
function tryParseQuiz(text) {
  try { const parsed = JSON.parse(text); if (typeof parsed === 'string') return JSON.parse(parsed); return parsed; } catch {
    const extracted = extractJson(text); if (!extracted) return null; try { const parsed2 = JSON.parse(extracted); if (typeof parsed2 === 'string') return JSON.parse(parsed2); return parsed2; } catch { return null; }
  }
}
...
function normalizeQuizSchema(quiz, { topic, difficulty }) {
  ...
  function makePlaceholder(idx) { const n = idx + 1; return { id: `q${n}`, question: `Pregunta ${n} sobre ${topic}`, options: ["Opción A", "Opción B", "Opción C", "Opción D"], correctIndex: 0, explanation: "" }; }
  for (let i = 0; i < qs.length && out.questions.length < 10; i++) {
    ...
    out.questions.push({ id: q.id || `q${out.questions.length + 1}`, question: (q.question || `Pregunta ${out.questions.length + 1} sobre ${topic}`).toString(), options: options.map(o => o.toString()), correctIndex: ci, explanation: (q.explanation || '').toString() });
  }
...
async function generateChatTitle(userPrompt, rawAssistantReply) {
  ...
  try {
    const r = await ai.models.generateContent({ model: "gemini-2.5-flash", contents: [ { role: 'user', parts: [{ text: `${instruction}\n\n${contextText}` }] } ], generationConfig: { temperature: 0.2 } });
...
app.get("/chats/sessions", validateToken, async (req, res) => { const userId = req.user.user_id; const sessions = await getUserChatSessions(userId); res.json({ sessions }); });
app.post("/chats/sessions", validateToken, async (req, res) => { const userId = req.user.user_id; const { name } = req.body || {}; const id = await createChatSession(userId, (name && name.trim()) || undefined); const session = await getChatSessionById(userId, id); res.json({ session }); });
app.get("/chats/:sessionId/messages", validateToken, async (req, res) => { const userId = req.user.user_id; const sessionId = req.params.sessionId; const session = await getChatSessionById(userId, sessionId); if (!session) return res.status(404).json({ error: "Sesión no encontrada" }); const messages = await getChatHistory(sessionId, 100); res.json({ session, messages }); });
app.delete("/chats/:sessionId", validateToken, async (req, res) => { const userId = req.user.user_id; const sessionId = req.params.sessionId; const success = await deleteChatSession(userId, sessionId); if (!success) return res.status(404).json({ error: "Sesión no encontrada" }); res.json({ message: "Sesión eliminada correctamente" }); });
app.delete("/chats", validateToken, async (req, res) => { const userId = req.user.user_id; const deletedCount = await deleteAllUserChatSessions(userId); res.json({ message: `${deletedCount} conversaciones eliminadas correctamente` }); });
app.delete("/user/delete-account", validateToken, async (req, res) => { const userId = req.user.user_id; const success = await deleteAllUserData(userId); if (!success) return res.status(500).json({ error: "Error al eliminar la cuenta" }); res.json({ message: "Cuenta eliminada correctamente" }); });

// Quizzes endpoints (trimmed)
app.post("/quizzes/generate", validateToken, async (req, res) => {
  const { topic, difficulty } = req.body;
  const userId = req.user.user_id;
  if (!topic || typeof topic !== 'string' || topic.trim().length < 3) return res.status(400).json({ error: "Proporciona un tema válido (mín. 3 caracteres)" });
  ...
  try {
    const response = await ai.models.generateContent({ model: "gemini-2.5-flash", contents: [ { role: "user", parts: [{ text: `Genera un quiz de practica en formato JSON estrictamente valido (sin markdown) sobre el tema: "${topic}".` }] } ], generationConfig: { responseMimeType: "application/json" } });
...
app.get("/quizzes/recent", validateToken, async (req, res) => { const userId = req.user.user_id; const items = await getRecentQuizzes(userId, 12); res.json({ items }); });
app.get("/quizzes/:id", validateToken, async (req, res) => { const userId = req.user.user_id; const quizId = req.params.id; const row = await getQuizById(quizId, userId); if (!row) return res.status(404).json({ error: "Quiz no encontrado" }); let questions; if (typeof row.questions_json === 'string') { try { questions = JSON.parse(row.questions_json); } catch (e) { questions = []; } } else if (Array.isArray(row.questions_json)) { questions = row.questions_json; } else { questions = []; } res.json({ id: row.id, topic: row.topic, difficulty: row.difficulty, created_at: row.created_at, questions }); });
app.post("/quizzes/:id/attempt", validateToken, async (req, res) => { const userId = req.user.user_id; const quizId = req.params.id; const { answers } = req.body; if (!Array.isArray(answers) || answers.length !== 10) return res.status(400).json({ error: "Debes enviar 10 respuestas" }); const row = await getQuizById(quizId, userId); if (!row) return res.status(404).json({ error: "Quiz no encontrado" }); let questions = []; try { questions = JSON.parse(row.questions_json); } catch {} const total = questions.length; let score = 0; questions.forEach((q, idx) => { if (typeof q.correctIndex === 'number' && answers[idx] === q.correctIndex) score += 1; }); const attemptId = await recordQuizAttempt(quizId, userId, answers, score, total); res.json({ attemptId, score, total, percentage: Math.round((score/total)*100) }); });

app.get("/progress/summary", validateToken, async (req, res) => { const userId = req.user.user_id; const summary = await getProgressSummary(userId); res.json(summary); });
